...
# def fast_trip_cost(cities, problem, dist_matrix):
#     if not cities:
#         return 0.0
...
#     # Start at Base (Index 0)
#     current_node = 0

#     # Path: 0 -> City 1 -> City 2 -> ... -> 0
#     path_targets = cities + [(0, 0)]

#     for target in path_targets:
#         target_id, target_gold = target
        
#         # O(1) Lookup of the TRUE distance
#         dist = dist_matrix[current_node][target_id]
        
#         # If the move is impossible (disconnected graph), penalty is infinite
#         if dist == float('inf'):
#             return float('inf')

#         # Cost Formula
#         # Note: This is an approximation for Beta > 1 because
#         # it treats the path as one 'hop'. However, since we now use
#         # real distances, the relative ranking of solutions will be correct.
#         step_cost = dist + (problem.alpha * dist * current_gold) ** problem.beta
#         cost += step_cost
...
# def enforce_canonical_order(trip_cities, dist_matrix):
#     """
#     Sorts a trip so we visit the node furthest from base (in Graph Distance) first.
#     """
#     # dist_matrix[0][c[0]] is the distance from Base (0) to City (c[0])
#     trip_cities.sort(key=lambda c: dist_matrix[0][c[0]], reverse=True)
    ...
    def run_parallel(self, total_generations=200, migration_interval=20):
        ...
        with ProcessPoolExecutor(max_workers=len(self.islands)) as executor:
            for epoch in range(num_epochs):
                ...
                # 4. REPORT: Print best global cost
